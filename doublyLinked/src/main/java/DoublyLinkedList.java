class DoublyLinkedList {
    ...
    // -------------------------------------------------------------
    public Link deleteFirst() { // (предполагается, что список не пуст)
        Link temp = first;
    ...
    // -------------------------------------------------------------
    public Link deleteLast() { // (предполагается, что список не пуст)
        Link temp = last;
        if(first.next == null) // Если только один элемент
    ...
    // -------------------------------------------------------------
    public Link deleteKey(long key) { // (предполагается, что список не пуст)
        Link current = first; // От начала списка
        while(current.dData != key) // Пока не будет найдено совпадение
        {
            current = current.next; // Переход к следующему элементу
